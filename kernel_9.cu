...
__global__ void kernel_9(const int4 *d_in, int *d_out, size_t N) {
...
void kernel_9_launch(const int *d_in, int *d_out, size_t N) {
  ...
  kernel_9<threadsPerBlock, batchSize><<<numBlocks, threadsPerBlock>>>(d_in_cast,
                                                                       d_out, N);
}
...
int main() {
  ...
  kernel_9_launch<threadsPerBlock, batchSize>(d_in, d_out, N);
